...
const fieldClass =
  "focus-ring w-full border-b border-stroke/20 bg-transparent px-0 py-3.5 text-body outline-none transition-colors placeholder:text-body-faint hover:border-stroke/40 focus:border-vermilion";

const labelClass = "font-mono text-[0.58rem] tracked-label text-body-faint";
        ...
        <AnimatePresence mode="wait">
          {reference ? (
            ...
            >
              ...
              <h2 className="mt-10 font-display text-4xl font-semibold tracking-tight text-body">
                Received.
              </h2>
              <p className="mt-4 max-w-sm text-sm leading-relaxed text-body-muted">
                We will look at your geometry and come back with pricing and a
                date — usually the same working day.
              </p>
              <p className="mt-6 font-mono text-[0.62rem] tracked-label text-body-soft">
                ...
                <div
                  ...
                  className={`group/zone relative flex min-h-[28rem] flex-col items-center justify-center gap-6 overflow-hidden border p-10 text-center transition-all duration-500 ${
                    dragging
                      ...
                      : "border-dashed border-stroke/25 hover:border-stroke/45 hover:bg-surface-warm/50"
                  }`}
                >
                  ...
                  >
                    <motion.span
                      ...
                      className="flex h-16 w-16 items-center justify-center rounded-full border border-stroke/20 text-body transition-colors duration-500 group-hover:border-vermilion group-hover:text-vermilion"
                    >
                    ...
                    <span className="block">
                      <span className="block font-display text-2xl text-body">
                        Drop your model here
                      </span>
                      <span className="mt-2 block font-mono text-[0.58rem] tracked-label text-body-faint">
                  ...
                  {files.length > 0 && (
                    <ul aria-live="polite" className="mt-2 w-full max-w-sm space-y-2 text-left">
                      {files.map((f, i) => (
                        ...
                        >
                          <span className="truncate text-xs text-body-soft">{f.name}</span>
                          <span className="flex shrink-0 items-center gap-3">
                            <span className="font-mono text-[0.55rem] text-body-faint">
                              {formatBytes(f.size)}
                            </span>
                            <button
                              type="button"
                              ...
                              className="focus-ring text-body-faint transition-colors hover:text-vermilion"
                            >
                              ×
                            </button>
                          </span>
