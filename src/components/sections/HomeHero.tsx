  ...
  return (
    ...
    >
      ...
      >
        ...
        <Enso className="absolute inset-0 h-full w-full" color="var(--stroke)" strokeWidth={5} />

        {/* specks flicked off the brush as it left the paper */}
        <Splatter className="absolute -right-[4%] top-[6%] h-40 w-40" color="var(--stroke)" />
      ...
      <div className="pointer-events-none absolute -right-14 bottom-[-3rem] h-[19rem] w-[19rem] opacity-50 sm:h-[24rem] sm:w-[24rem] lg:hidden">
        <MoonWindow className="absolute inset-0 h-full w-full" />
        <Enso className="absolute inset-0 h-full w-full" color="var(--stroke)" strokeWidth={6} />
      </div>
      ...
      <div className="absolute left-6 top-1/2 hidden -translate-y-1/2 lg:block">
        ...
        >
          ...
          <span className="vertical-label font-mono text-[0.6rem] text-body-muted">
            Print Studio
          </span>
          <span className="h-20 w-px bg-rule-strong" />
        </motion.div>
      ...
      >
        <div className="max-w-2xl">
          ...
          <h1 className="mt-8 font-display text-[3.4rem] font-semibold leading-[0.98] tracking-tight text-body sm:text-7xl lg:text-[6.2rem]">
            {line.map((word, i) => (
          ...
          <div className="mt-8 h-5 w-64">
            <BrushStroke variant="swash" color="var(--stroke)" className="h-full w-full" opacity={0.9} />
          </div>
          ...
          <motion.p
            ...
            className="mt-8 max-w-md text-[1.05rem] leading-relaxed text-body-muted"
          >
      ...
      >
        <span className="font-mono text-[0.55rem] tracked-label text-body-faint">Scroll</span>
        <motion.span
          animate={{ scaleY: [0.3, 1, 0.3], originY: 0 }}
          transition={{ duration: 2.2, repeat: Infinity, ease: "easeInOut" }}
          className="h-12 w-px bg-stroke/30"
        />
      </motion.div>
    </section>
  );
